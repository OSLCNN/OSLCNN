...
def construct_model(model_name, height_in, c_in, path = ""):
    file_name = model_name
    if not (len(model_name) > 5 and model_name[-5:] == ".json"):
        file_name = path + './json/' + model_name + '.json'
            ...
            for index, child_nodes in enumerate(current_node.child_nodes):
                ...
                if len(child_node.parent_nodes) == 1:
                    ...
                    while len(child_node.parent_nodes) == 1:
                        ...
                        before_node = child_node
                        if child_node.child_nodes.__len__() == 0:
                            print(child_node)
...
if __name__ == '__main__':
    ...
    test_nodes = construct_model("YOLO", 448, 3)
    print(get_sort_nodes(test_nodes[0]))
