...
from On_demand_Fine_grained_Partitioning.Util import get_fitness2
...
class PSO:
    ...
    def run_adv(self):
        ...
        self.run_pso(initPop, initVec)
        if self.algorithm == 'PSOGA':
            global_fitness_list[-1] = 1 / get_fitness2('',self.nodes, global_best_decision)
        return global_best_decision, fitness_list, global_fitness_list
    ...
    def find_best(self, pops):
        ...
        for i in range(len(pops)):
            fitness = 1 / get_fitness2(self.algorithm,self.nodes,  pops[i])
    ...
    def run_pso(self, pop, vec):
        ...
        for i in range(len(pop)):
            fitness = 1 / get_fitness2(self.algorithm,self.nodes,  pop[i])
        ...
        for i in range(self.max_iter_size):   # 迭代更新速度和位置
            ...
            for j in range(len(pop)):
                father = pop[j]
                mothor_index = np.random.randint(0,pop.__len__())
                mothor = pop[mothor_index]
                left = np.random.randint(0,pop[0].__len__()-2)
                right = np.random.randint(left+1, pop[0].__len__()-1)
                newchild = father[:left]+mothor[left:right]+father[right:]
                pop[j] = newchild
            #变异
            for j in range(len(pop)):
                for jj in range(len(pop[0])):
                    if np.random.rand()<0.05:
                        pop[j][jj] = np.random.randint(0,self.deviceNum+self.cloudNum+self.edgeNum)

            for j in range(len(pop)):
                fitness = 0
                if np.random.rand() < parameters.cross_rate:
                    fitnessGA = 1 / get_fitness2(self.algorithm, self.nodes, pop[j])
                    if pBestFitness[j] > fitnessGA:
                        pBestFitness[j] = fitnessGA
                        pBestList[j] = copy.copy(pop[j])
                    fitness = fitnessGA
                else:
                    fitnessPSO = 1 / get_fitness2(self.algorithm, self.nodes, popPSO[j])
                    if pBestFitness[j] > fitnessPSO:
                        pBestList[j] = popPSO[j]
                        pBestFitness[j] = fitnessPSO
                    pop[j] = popPSO[j]
                    fitness = fitnessPSO
