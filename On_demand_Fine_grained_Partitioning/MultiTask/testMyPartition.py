...
from On_demand_Fine_grained_Partitioning.MultiTask.PSOPartition0 import PSO
...
def run_fix_bp(devices_p, B_p, nodes_p, leftTime, nowTime):
    ...
    # plt.show()
    return time_list, decision_list[0]
...
if __name__ == '__main__':
    ...
    nodes = models[0].nodes
    print("长度", nodes.__len__())

    # 1. 固定环境，可以 单独 运行各个模型，分别比较 收敛程度+++，时延
    devices, B = ConstructEnvironment.construct_environment_real() # 指定环境
    p = time.time()
    sum = 0
    #test_partition(nodes,'133')
    run_fix_bp(devices, B, nodes)
