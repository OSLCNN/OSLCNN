...
from On_demand_Fine_grained_Partitioning.MultiTask.MyUtil import get_fitness1, getTimes
...
class PSO:
    ...
    def find_best(self, pops):
        ...
        for i in range(len(pops)):
            fitness = 1 / get_fitness1(self.nodes,  pops[i])
    ...
    def run_pso(self, pop, vec):
        ...
        for i in range(len(pop)):
            fitness = 1 / get_fitness1(self.nodes,  pop[i])
        ...
        for i in range(self.max_iter_size):   # 迭代更新速度和位置
            ...
            pBest = 0
            for j in range(len(pop)):
                fitness = 1 / get_fitness1(self.nodes,  pop[j])
