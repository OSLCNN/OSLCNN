...
class TSP:
    ...
    def __init__(self, citys, maxgen=20000,
    ...
    def result_pa(self,i):
        self.index = np.argmax(self.fitness)
        print(i,"the total distance is", self.dis[self.index])
...
if __name__ == '__main__':
    ...
    randomcity = []
    for i in range(20):
        ranx = np.random.randint(0,100)
        rany = np.random.randint(0, 100)
        ran = []
        ran.append(ranx)
        ran.append(rany)
        randomcity.append(ran)
