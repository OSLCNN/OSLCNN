...
from On_demand_Fine_grained_Partitioning.Util import get_fitness
...
class PSO:
    ...
    def run(self):
        ...
        global_best_decision = copy.deepcopy(current_best_decision)

        self.run_pso(initPop, initVec)
        if self.algorithm == 'PSOGA':
            global_fitness_list[-1] = 1 / get_fitness('', self.nodes, global_best_decision)
    ...
    def find_best(self, pops):
        ...
        for i in range(len(pops)):
            fitness = 1 / get_fitness(self.algorithm, self.nodes, pops[i])
            if fitness < best_fitness:
                best_fitness = fitness
                best_index = i
        return copy.deepcopy(pops[best_index]), best_fitness
        ...
        for i in range(self.max_iter_size):   # 迭代更新速度和位置
            ...
            # v(t) = w * v(t - 1) + c1 * r1 * (gbest(t - 1)) + c2 * r2 * (pbest(t - 1))  速度更新公式
                for k in range(len(vec1)):      # 计算速度
                    vec1[k] = int(0.5 * vec1[k] + random.random() * 2 * (abs(global_best_decision[k] - pop[j][k])) +
            ...
            for j in range(len(pop)):
                fitness = 1 / get_fitness(self.algorithm, self.nodes, pop[j])

                if pBestFitness[j] > fitness:
                    pBestList[j] = copy.deepcopy(pop[j])
                    pBestFitness[j] = fitness
                if global_best_fitness > fitness:      # 更新全局最优解gBest
                    global_best_fitness = fitness
                    global_best_decision = copy.deepcopy(pop[j])

            de, pBest = self.find_best(pop)
