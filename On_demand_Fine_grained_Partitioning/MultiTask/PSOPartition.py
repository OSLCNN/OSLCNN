...
class PSOPartition:
    def __init__(
        ...
        for j in range(models.__len__()):
            ...
            self.psoLength += nodes.__len__() - 1
                ...
                while child_i < len(begin_node_child_nodes):
                    ...
                    if not isinstance(child_node, ConvConcatNode):  # 孩子不是卷积层
                        ...
                        if getTimeIndex == 1:
                            ...
                            self.transFlops += MyUtil.timeTrans(begin_node,child_node.vmId) * self.devices[child_node.vmId].p
                            self.waitFlops += max(0, time - deviceTime[child_node.vmId]) * self.devices[child_node.vmId].p
                        ...
                        if getTimeIndex == 1:
                            self.transTime += MyUtil.timeTrans(child_node, nextVmId)
                            self.transFlops += MyUtil.timeTrans(child_node, nextVmId) * self.devices[nextVmId].p
                        child_i += 1
                        # 计算时间，卷积层的
